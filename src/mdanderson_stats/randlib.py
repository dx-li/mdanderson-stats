...
from ._randlib_multinomial import probabilities, sample_multinomial
...
class RandlibGenerator:
    ...
    def multinomial(
        ...
        *,
        ...
        """Multinomial category counts (GENMUL); p includes every category."""
        n = _integer(n, "n", 0, 2**53 - 1)
        size = _integer(size, "size", 0, self._max_draws)
        if not isinstance(legacy, (bool, np.bool_)):
            raise ValueError("legacy must be boolean")
        ...
        if not legacy and source != "fortran":
            raise ValueError("source selection requires legacy=True")
        p = probabilities(p, self._max_draws)
        if size * len(p) > self._max_draws:
            raise ValueError("size * number of categories must be <= max_draws")
        budget = _integer(
            max(100_000, 8 * size * (len(p) - 1)) if max_attempts is None else max_attempts,
            "max_attempts",
            1,
            2**53 - 1,
        )
        result, state = sample_multinomial(
            self.get_seeds(),
            self._antithetic[self._stream - 1],
            size,
            n,
