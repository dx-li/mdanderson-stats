"""SMO expected-likelihood power for logistic and Poisson regression designs."""
...
def asypow_smo_regression(
    ...
    *,
    ...
    """Linear/quadratic logistic or log-mean Poisson regression SMO.
    ...
    log_scale = float(logsumexp(log_weight + (eta if family == "poisson" else log_p + log_s)))

    def likelihood(_p: FloatArray, q: FloatArray) -> float:
        candidate = predictors(q)
        if family == "poisson":
            log_kl = eta + _log_exp_remainder(candidate - eta)
        else:
    ...
    def gradient(_p: FloatArray, q: FloatArray) -> FloatArray:
        ...
        if family == "poisson":
            log_score = log_weight + eta + log_difference - log_scale
        else:
