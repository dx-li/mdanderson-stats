"""Measure checked legacy binomial batching against repeated scalar calls."""
...
def main():
    cases = []
    for size in (64, 256):
        s = np.linspace(0, 15, size)
        forward = cdfbin(s=s, n=20, pr=0.5)
        for mode, name in [(1, "p"), (2, "s"), (3, "n"), (4, "pr")]:

            def evaluate(index):
                kwargs = dict(s=s[index], n=20, pr=0.5)
                if mode != 1:
                    kwargs.pop(name)
                    kwargs.update(p=forward.p[index], q=forward.q[index])
                return getattr(cdfbin(mode, **kwargs), name)
    ...
    result = dict(
        ...
        repetitions=3,
        inputs="n=20, pr=.5, s=linspace(0,15,size)",
        comparison=(
            "One broadcast call versus n scalar calls to the same Python API; not native C/Fortran"
        ),
        cases=cases,
    )
    Path("docs/dcdflib-binomial-benchmark.json").write_text(json.dumps(result, indent=2) + "\n")
