"""Record KSBIN1's null-probability solve mode using the documented reference build."""
...
read(*,*) params(2),direction,params(3),params(5)
params(1)=0.5_dpkind
params(4)=0.05_dpkind
call solve_binomial_one_sample(1,params,direction==1,status)
write(*,'(I4,3(1X,ES27.17E3))') status,params(1),params(4),params(5)
end program
""")
    ...
    cases = []
    for n in [30, 50, 100]:
        for pa, direction in [(0.06, 0), (0.6, 1), (0.94, 1)]:
            for target in [0.5, 0.8, 0.95]:
                result = subprocess.run(
                    [str(executable)],
                    input=f"{pa} {direction} {n} {target}\n",
                    text=True,
                    capture_output=True,
                    check=True,
                    timeout=10,
    ...
    provenance["source"] = "KSBIN1 solve_binomial_one_sample mode 1; unchanged statistical routines"
    provenance["cases"] = cases
    Path("tests/fixtures/binomial_null.json").write_text(json.dumps(provenance, indent=2) + "\n")
    print(f"Recorded {len(cases)} native null-probability solves")
