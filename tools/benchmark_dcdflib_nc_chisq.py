"""Measure checked legacy noncentral chi-square batching against repeated scalar calls."""
...
from mdanderson_stats import cdfchn


def main():
    cases = []
    for size in (64, 256):
        x = np.linspace(0.2, 20, size)
        forward = cdfchn(x=x, df=2, pnonc=4)
        for mode, name in [(1, "p"), (2, "x"), (3, "df"), (4, "pnonc")]:

            def evaluate(index):
                kwargs = dict(x=x[index], df=2, pnonc=4)
                if mode != 1:
                    kwargs.pop(name)
                    kwargs.update(p=forward.p[index], q=forward.q[index])
                return getattr(cdfchn(mode, **kwargs), name)
    ...
    result = dict(
        ...
        repetitions=3,
        inputs="df=2, pnonc=4, x=linspace(.2,20,size)",
        comparison=(
            "One broadcast call versus n scalar calls to the same Python API; not native C/Fortran"
        ),
        cases=cases,
    )
    Path("docs/dcdflib-nc-chisq-benchmark.json").write_text(json.dumps(result, indent=2) + "\n")
