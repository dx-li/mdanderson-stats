"""Compare legacy noncentral F batching with repeated scalar calls to the same Python API."""
...
from mdanderson_stats import cdffnc
...
def main():
    ...
    for size in (64, 256):
        ...
        r = cdffnc(f=1, dfn=nn, dfd=10, pnonc=4)
        for mode in (1, 3, 4):

            def evaluate(index):
                if mode == 1:
                    return cdffnc(f=1, dfn=nn[index], dfd=10, pnonc=4).p
                if mode == 3:
                    return cdffnc(
                        3,
                        p=r.p[index],
                ...
                return cdffnc(
                    4, p=r.p[index], q=r.q[index], f=1, dfn=nn[index], pnonc=4, df_bracket=(8, 12)
                ).dfd
    ...
    result = {
        ...
        "repetitions": 3,
        "inputs": (
            "f=1, dfn=linspace(2,10,n), dfd=10, pnonc=4; "
    ...
    Path("docs/dcdflib-nc-f-benchmark.json").write_text(json.dumps(result, indent=2) + "\n")
