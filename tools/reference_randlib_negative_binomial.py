"""Native C/Fortran negative_binomial values and consumed generator states."""
...
def main():
    ...
    work = Path("research/raw/reference/randlib-negative_binomial").resolve()
...
"""
        ...
        provenance[language] = {
            ...
            "flags": flags,
            "storage_note": (
                "Fortran static locals retain IGNPOI cumulative table; C table is already static"
            ),
            "source_sha256": {
            ...
            for stream in [1, 32]:
                for antithetic in [False, True]:
                    for n, p in pairs:
                        ...
                        if len(rows) != 20:
                            raise ValueError("expected 20 negative_binomial draws")
                        cases.append(
                            dict(
                                language=language,
                                seed=seed,
    ...
    Path("tests/fixtures/randlib_negative_binomial.json").write_text(
        json.dumps(dict(provenance=provenance, cases=cases), indent=2) + "\n"
    )
    print(f"Generated {len(cases)} native negative_binomial cases")
