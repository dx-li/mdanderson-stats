"""Native noncentral F evidence, retaining original and repaired profiles."""
...
real(8) p,q,f,dfn,dfd,nc
read(*,*) which,p,q,f,dfn,dfd,nc
call cdf_nc_f(which,cum=p,ccum=q,f=f,dfn=dfn,dfd=dfd,pnonc=nc,status=status)
write(*,'(I5,6ES26.17E3)') status,p,q,f,dfn,dfd,nc
...
def main():
    ...
    for profile in ["original", "central_status_repaired"]:
        work = Path("research/raw/reference/cdflib-nc-f", profile).resolve()
        work.mkdir(parents=True, exist_ok=True)
        compiled = []
        patches = []
        for path in paths:
            raw = path.read_bytes()
            if profile != "original" and path.stem == "cdf_f_mod":
    ...
    for f, dfn, dfd, nc in itertools.product(
        (0.1, 1.0, 10.0), (0.5, 2.0, 10.0), (2.0, 10.0), (0.0, 0.5, 4.0, 20.0)
    ):
        inputs = [1, 0.0, 0.0, f, dfn, dfd, nc]
        ...
        if not isinstance(p, float) or not isinstance(q, float) or min(p, q) < 1e-5:
            continue
        for which in (2, 3):
            if which == 3 and nc == 0:
                continue
            inputs = [which, p, q, f, dfn, dfd, nc]
            for profile in profiles:
                status, result = run(profile, inputs)
    ...
    output = {
        ...
        "driver": DRIVER,
        "input_order": ["which", "cum", "ccum", "f", "dfn", "dfd", "pnonc"],
        "result_order": ["cum", "ccum", "f", "dfn", "dfd", "pnonc"],
        "profiles": profiles,
    }
    Path("tests/fixtures/cdflib_nc_f.json").write_text(
