"""Compile both archived legacy languages and record their Poisson contracts."""
...
#include "cdflib.h"
int main(void) {
 int which,status; double p,q,s,mean,bound=0;
 if(scanf("%d %lf %lf %lf %lf",&which,&p,&q,&s,&mean)!=5) return 1;
 cdfpoi(&which,&p,&q,&s,&mean,&status,&bound);
 printf("%d %.17g %.17g %.17g %.17g %.17g\n",status,p,q,s,mean,bound);
 return 0;
}
"""
...
real(8) p,q,s,mean,bound
read(*,*) which,p,q,s,mean
bound=0
call cdfpoi(which,p,q,s,mean,status,bound)
write(*,'(I5,5ES26.17E3)') status,p,q,s,mean,bound
...
def main():
    ...
    for language in ("c", "fortran"):
        work = Path("research/raw/reference/dcdflib-poisson", language).resolve()
    ...
    for s, mean in itertools.product((0.0, 0.1, 0.5, 1.0, 5.0, 20.0), (0.0, 0.1, 1.0, 10.0, 100.0)):
        inputs = [1, 0.0, 0.0, s, mean]
    ...
    result = {
        ...
        "input_order": ["which", "p", "q", "s", "mean"],
        "result_order": ["p", "q", "s", "mean", "bound"],
        "bound_policy": (
            "Driver initializes bound to zero; only defined by native contract on failure."
        ),
        "profiles": profiles,
    }
    Path("tests/fixtures/dcdflib_poisson.json").write_text(
