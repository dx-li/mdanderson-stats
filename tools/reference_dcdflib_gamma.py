"""Compile both archived legacy languages and record their gamma contracts."""
...
#include "cdflib.h"
int main(void) {
 int which,status; double p,q,x,shape,rate,bound=0;
 if(scanf("%d %lf %lf %lf %lf %lf",&which,&p,&q,&x,&shape,&rate)!=6) return 1;
 cdfgam(&which,&p,&q,&x,&shape,&rate,&status,&bound);
 printf("%d %.17g %.17g %.17g %.17g %.17g %.17g\n",status,p,q,x,shape,rate,bound);
 return 0;
}
"""
...
real(8) p,q,x,shape,rate,bound
read(*,*) which,p,q,x,shape,rate
bound=0
call cdfgam(which,p,q,x,shape,rate,status,bound)
write(*,'(I5,6ES26.17E3)') status,p,q,x,shape,rate,bound
...
def main():
    ...
    for language in ("c", "fortran"):
        work = Path("research/raw/reference/dcdflib-gamma", language).resolve()
    ...
    for x, shape, rate in itertools.product(
        (0.0, 0.1, 1.0, 10.0), (0.2, 1.0, 2.0, 10.0), (0.5, 2.0)
    ):
        inputs = [1, 0.0, 0.0, x, shape, rate]
        ...
        for which in (2, 3, 4):
            if q == 0 or (which in (3, 4) and p == 0):
                continue
            inputs = [which, p, q, x, shape, rate]
            for language in profiles:
                status, result = run(language, inputs)
                profiles[language]["cases"].append(
                    {"input": inputs, "status": status, "result": result}
    ...
    for language in profiles:
        ...
        p, q = -math.expm1(-1.0), math.exp(-1.0)
        profiles[language]["wide_cases"] = []
        for inputs in (
            [1, 0, 0, 1e200, 1, 1e-200],
            [2, p, q, 1, 1, 1e-200],
            [4, p, q, 1e-200, 1, 1],
            [1, 0, 0, 1e-200, 0.5, 1e-200],
            [2, 1e-200, 1, 1, 0.5, 1e-308],
            [4, 0, 1, 1, 2, 1],
        ):
            status, result = run(language, inputs)
    ...
    result = {
        ...
        "input_order": ["which", "p", "q", "x", "shape", "rate"],
        "result_order": ["p", "q", "x", "shape", "rate", "bound"],
        "bound_policy": (
            "Driver initializes bound to zero; only defined by native contract on failure."
        ),
        "profiles": profiles,
    }
    Path("tests/fixtures/dcdflib_gamma.json").write_text(
