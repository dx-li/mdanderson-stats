"""Native noncentral t evidence, retaining original and repaired profiles."""
...
call cdf_nc_t(which,cum=p,ccum=q,t=t,df=df,pnonc=nc,status=status)
write(*,'(I5,5ES26.17E3)') status,p,q,t,df,nc
...
def main():
    ...
    for profile in ["original", "central_status_repaired"]:
        work = Path("research/raw/reference/cdflib-nc-t", profile).resolve()
        work.mkdir(parents=True, exist_ok=True)
        compiled = []
        patches = []
        for path in paths:
            raw = path.read_bytes()
            if profile != "original" and path.stem == "cdf_t_mod":
    ...
    for t, df, nc in itertools.product(
        (-2.0, 0.0, 0.5, 2.0, 5.0), (0.5, 2.0, 10.0), (0.0, 0.5, 2.0, 4.0)
    ):
        ...
        if not isinstance(p, float) or not isinstance(q, float) or min(p, q) < 1e-5:
            continue
        for which in (2, 3, 4):
            if (which == 3 and t == 0) or (which == 4 and nc == 0):
                continue
    ...
    for profile in profiles:
        inputs = [3, 0.03, 0.97, 1.0, 5.0, 3.0]
        status, result = run(profile, inputs)
        profiles[profile]["multiple_root_case"] = {
            "input": inputs,
            "status": status,
            "result": result,
        }
    ...
    output = {
        ...
        "driver": DRIVER,
        "input_order": ["which", "cum", "ccum", "t", "df", "pnonc"],
        "result_order": ["cum", "ccum", "t", "df", "pnonc"],
        "profiles": profiles,
    }
    Path("tests/fixtures/cdflib_nc_t.json").write_text(
