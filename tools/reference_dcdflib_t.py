"""Compile both archived legacy languages and record their Student t contracts."""
...
#include "cdflib.h"
int main(void) {
 int which,status; double p,q,t,df,bound=0;
 if(scanf("%d %lf %lf %lf %lf",&which,&p,&q,&t,&df)!=5) return 1;
 cdft(&which,&p,&q,&t,&df,&status,&bound);
 printf("%d %.17g %.17g %.17g %.17g %.17g\n",status,p,q,t,df,bound);
 return 0;
}
"""
...
real(8) p,q,t,df,bound
read(*,*) which,p,q,t,df
bound=0
call cdft(which,p,q,t,df,status,bound)
write(*,'(I5,5ES26.17E3)') status,p,q,t,df,bound
...
def main():
    ...
    for language in ("c", "fortran"):
        work = Path("research/raw/reference/dcdflib-t", language).resolve()
    ...
    for t, df in itertools.product(
        (-10.0, -2.0, -0.1, 0.0, 0.1, 2.0, 10.0), (0.2, 1.0, 2.0, 10.0, 40.0)
    ):
    ...
    for language in profiles:
        ...
        for t, df in ((1e200, 1), (1e200, 0.1), (1, 1e-100), (1, 1e100), (1e-200, 1)):
            inputs = [1, 0, 0, t, df]
            status, result = run(language, inputs)
    ...
    result = {
        ...
        "input_order": ["which", "p", "q", "t", "df"],
        "result_order": ["p", "q", "t", "df", "bound"],
        "bound_policy": (
            "Driver initializes bound to zero; only defined by native contract on failure."
        ),
        "profiles": profiles,
    }
    Path("tests/fixtures/dcdflib_t.json").write_text(
