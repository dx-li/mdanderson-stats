"""Measure checked legacy negative-binomial batching against repeated scalar calls."""
...
def main():
    cases = []
    for size in (64, 256):
        f = np.linspace(0, 20, size)
        forward = cdfnbn(f=f, s=2, pr=0.5)
        for mode, name in [(1, "p"), (2, "f"), (3, "s"), (4, "pr")]:

            def evaluate(index):
                kwargs = dict(f=f[index], s=2, pr=0.5)
                if mode != 1:
                    kwargs.pop(name)
                    kwargs.update(p=forward.p[index], q=forward.q[index])
                return getattr(cdfnbn(mode, **kwargs), name)
    ...
    result = dict(
        ...
        repetitions=3,
        inputs="s=2, pr=.5, f=linspace(0,20,n)",
        comparison=(
            "One broadcast call versus n scalar calls to the same Python API; not native C/Fortran"
        ),
        cases=cases,
    )
    Path("docs/dcdflib-neg-binomial-benchmark.json").write_text(json.dumps(result, indent=2) + "\n")
