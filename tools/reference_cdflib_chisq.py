"""Original CDFLIB90 chisq forward and inverse reference cases."""
...
real(8) p,q,x,df
read(*,*) which,p,q,x,df
call cdf_chisq(which,cum=p,ccum=q,x=x,df=df,status=status)
write(*,'(I5,4ES26.17E3)') status,p,q,x,df
...
def main():
    ...
    work = Path("research/raw/reference/cdflib-chisq").resolve()
    ...
    cases = []
    for x, df in itertools.product((0.01, 0.2, 1.0, 5.0, 30.0), (0.001, 0.2, 1.0, 2.0, 5.0, 20.0)):
        ...
        if min(values[:2]) < 1e-8:
            continue
        for which in (2, 3):
            inputs = [which, values[0], values[1], x, df]
    ...
    output = {
        ...
        "driver": DRIVER,
        "adaptations": [],
        "input_order": ["which", "cum", "ccum", "x", "df"],
        "result_order": ["cum", "ccum", "x", "df"],
        "cases": cases,
    }
    target = Path("tests/fixtures/cdflib_chisq.json")
    target.write_text(json.dumps(output, indent=2, allow_nan=False) + "\n")
    print(f"Recorded {len(cases)} chisq cases")
