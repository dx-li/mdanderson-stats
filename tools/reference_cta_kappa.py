"""Native CTA KAPPA output; source remains an unbundled local reference."""
...
def main():
    ...
    work = Path("research/raw/reference/cta-kappa").resolve()
...
implicit none
real ob(100,100),prop(100,100),k,v1,v2
integer n,i
read(*,*)n
ob=0
do i=1,n
...
call kappa(ob,prop,n,100,k,v1,v2)
write(*,'(A,3ES24.15)')'RESULT ',k,v1,v2
end program
""")
    ...
    tables.extend([[[20, 2], [9, 1]], [[20, 9], [2, 1]]])
    cases = []
    for table in tables:
    ...
    fixture = {
        ...
        "columns": ["kappa", "variance", "null_variance"],
        "cases": cases,
    }
    Path("tests/fixtures/cta_kappa.json").write_text(json.dumps(fixture, indent=2) + "\n")
    print(f"Generated {len(cases)} native KAPPA cases")
