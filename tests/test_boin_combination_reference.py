...
def test_native_boundary_table_matches_all_patient_counts():
    ...
    expected_elim = [int(v["eliminate_if_dlt_ge"]) if v["eliminate_if_dlt_ge"] != "NA" else 0 for v in rows]
    # The public boundary object uses n+1 for an impossible elimination cutoff.
    assert table.eliminate_min.tolist() == [x or n + 1 for x, n in zip(expected_elim, table.patients)]
...
def test_native_movement_cases_preserve_axis_neighbors_and_safety():
    ...
    for row in rows:
        ...
        result = design.next_dose(
            _matrix(row, "patients"), _matrix(row, "toxicities"),
            (int(row["current_a"]), int(row["current_b"])), rng=1,
        )
        ...
        # compared exactly.
        if row["case"] in {"escalate_equal_neighbor_tie", "deescalate_to_higher_posterior_neighbor"}:
...
def test_selection_uses_native_rounded_fit_and_cross_closure():
    rows = {row["case"]: row for row in csv.DictReader((FIXTURES / "boin-combination-selection.csv").open())}
    design = BOINCombDesign()
    ...
    assert_array_equal(result.eliminated, np.asarray([
    ...
    result = design.select_mtd(
        _matrix(row, "patients"), _matrix(row, "toxicities"), bound_mtd=True
    )
    assert result.dose == (1, 2)


def test_contour_matches_all_native_rows_and_is_monotone():
    row = next(csv.DictReader((FIXTURES / "boin-combination-selection.csv").open()))
    ...
    assert result.contour == ((1, 2), (2, 3), (3, 3))
    assert all(a[1] <= b[1] for a, b in zip(result.contour, result.contour[1:]))


def test_all_toxic_lowest_dose_is_a_clean_no_mtd_result():
    row = next(
        r for r in csv.DictReader((FIXTURES / "boin-combination-selection.csv").open())
