...
def test_allocation_is_a_probability_vector(method):
    ...
    if method == "barn2n":
        kwargs["max_n"] = 50
    if method == "dbcd":
        kwargs.update(target_probability=[.2, .3, .5], tau=2, tau1=.5)
...
def test_floors_and_stopped_arms_are_applied_by_water_filling():
    result = barpo_allocation(
        posterior(), [5, 8, 12], method="barcp", tau=.7,
        stopped=[False, True, False], minimum_probability=[.15, 0, .15],
    )
    assert result[1] == 0
    assert result[0] >= .15 and result[2] >= .15
    np.testing.assert_allclose(result.sum(), 1)
...
def test_dbcd_rejects_zero_assigned_proportion_without_smoothing():
    with pytest.raises(ValueError, match="zero assigned"):
        barpo_allocation(
            ...
            [5, 0, 12], method="dbcd", target_probability=[.2, .3, .5]
        )


def test_monitoring_uses_strict_futility_and_inclusive_efficacy_cutoffs():
    result = barpo_monitor(
        [1, 4, 8], [4, 2, 1], prior=[[1, 1]] * 3,
        theta_fut=.25, pfut=.8, theta_eff=.65, peff=.8,
        theta_final=.5, pfinal=.8,
    )
    assert result.futile is not None and result.efficacious is not None
    np.testing.assert_array_equal(result.futile, result.futility_probability > .8)
    np.testing.assert_array_equal(result.efficacious, result.efficacy_probability >= .8)
    np.testing.assert_array_equal(result.final_efficacious, result.final_efficacy_probability >= .8)


def test_control_monitoring_uses_posterior_ordering():
    result = barpo_monitor(
        [2, 8], [8, 2], prior=[[1, 1]] * 2, control=True,
        pfut=.8, peff=.8, pfinal=.8,
    )
    assert result.futility_probability[0] == 0
    assert result.efficacy_probability[0] == 0
    assert result.efficacy_probability[1] > .5
