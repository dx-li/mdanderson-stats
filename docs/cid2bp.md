...
`cid2bp_interval` implements four methods from catalog entry **38**,
...
Focused tests check the supplied native reference output for all four
...
1e-4 solver tolerance. The original executable has not been rebuilt or run in
this validation; its supplied reference output and computational source were used.

**Catalog status remains partial.** Peskun's full lattice-based method (4),
weighted likelihood mid-P (6), conservative weighted likelihood (7), automatic
selection (8), exact intervals (9), and native session/report interfaces remain
pending. The implemented boundary adjustment is distinct from full method 4.
